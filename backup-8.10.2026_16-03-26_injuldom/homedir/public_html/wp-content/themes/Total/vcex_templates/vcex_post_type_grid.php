	...
	$output .= '<div class="'. $wrap_classes .'"'. vcex_get_unique_id( $unique_id ) .'>';
		...
		$output .= '<div class="'. $grid_classes .'"'. $grid_data .'>';
			...
			while ( $wpex_query->have_posts() ) :
				...
				$atts['post_excerpt']      = '';
				$atts['post_thumbnail_id'] = get_post_thumbnail_id( $atts['post_id'] );
					...
					$output .= '<div class="'. $classes .'">';
						...
						if ( 'true' == $entry_media ) :
							...
							elseif ( $latts['post_thumbnail_id'] ) :
								...
								$media_link = $atts['post_permalink'];
								...
								$output .= '<div class="'. $media_classes .'">';
									...
									if ( $thumb_link == 'post' || $thumb_link == 'lightbox' ) :
										...
										if ( $thumb_link == 'lightbox' ) {
											...
											$latts['lightbox_link'] = wpex_get_lightbox_image( $latts['post_thumbnail_id'] );
											$media_link_classes     = 'wpex-lightbox';

											// Video lightbox
											$lightbox_video_url = wpex_sanitize_data( wpex_get_post_video_oembed_url( $atts['post_id'] ), 'embed_url' );
											if ( $lightbox_video_url ) {
												$latts['lightbox_link']                 = $lightbox_video_url;
												$latts['lightbox_data']['data-type']    = 'data-type="iframe"';
												$latts['lightbox_data']['data-options'] = 'data-options="iframeType:\'video\',thumbnail:\''. $latts['lightbox_link'] .'\'"';
											}
											$media_link = $latts['lightbox_link'];
										}

										$lightbox_data      = ! empty( $latts['lightbox_data']  ) ? ' '. implode( ' ', $latts['lightbox_data'] ) : '';
										$media_link_classes = ! empty( $media_link_classes ) ? ' class="'. $media_link_classes .'"' : '';

										$output .= '<a href="'. $media_link .'" title="'. $latts['post_esc_title'] .'"'. $url_target .''. $media_link_classes . $lightbox_data .'>';
